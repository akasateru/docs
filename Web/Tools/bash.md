...
`set -o vi` で、シェルの行編集モードを既定のemacsモードからviモードに切り替えられる。

```bash
set -o vi     # viモードを有効化
set -o emacs  # emacsモード（デフォルト）に戻す
```

- 有効化すると、コマンドライン入力でviのキーバインディングが使える（`Esc`でノーマルモードに入り、`h`/`l`で移動、`w`/`b`で単語移動、`dd`で行削除など。詳細は[vim.md](vim.md)参照）
- 一時的な有効化はターミナルでそのままコマンド実行、永続化したい場合は `~/.bashrc`（zshなら `~/.zshrc`）に追記する
- fishシェルには同等の設定として `fish_vi_key_bindings` がある

エディタ（VSCodeなど）のキーバインドをviに統一している場合、シェル側もこの設定で揃えると操作感が一貫する。
