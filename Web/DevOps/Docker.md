...
### 1.6.3. Windows(WSL2)でディスク使用量が肥大化しWindows全体が重くなる

FastAPI・Nuxt・DBなど複数コンテナを常時起動していると、Windows自体のディスク使用量が増え続けPCが重くなることがある。主因は以下。

- **WSL2の仮想ディスク（`ext4.vhdx`）が肥大化する**: `docker system prune`等でDocker内部のデータを削除しても、確保済みの`.vhdx`ファイル自体は自動で縮小されない（一度膨らむと確保されたまま）。
- **Docker のビルドキャッシュ・不要イメージの蓄積**: 開発中に何度もビルドすると、レイヤーキャッシュ（[1.11.3](#1113-イメージの正体レイヤーとunion-filesystem)参照）が際限なく溜まる。
- **DBのボリュームデータの蓄積**: Postgres/MySQL等のボリュームが肥大化する。
- **メモリ/CPU上限を未設定のままDB+API+フロントを同時起動**: WSL2側のリソース上限（[WSL2 - .wslconfig設定](../Tools/WSL2.md#1-wslconfig設定)参照）を設定していないと、WSL2がホストWindowsのリソースを圧迫し全体が重くなる。

対処法:

1. `docker system df` で内訳（イメージ/コンテナ/ボリューム/ビルドキャッシュ）を確認する。
2. `docker system prune -a --volumes` で未使用のイメージ・コンテナ・ボリューム・ビルドキャッシュを削除する（使用中のvolumeは消えないが、`-a`は未使用イメージも全部消すため内容を確認してから実行する）。
3. `.vhdx`自体を縮小するには、`wsl --shutdown`でWSL2を完全停止した後、Windows側で`diskpart`を使い該当vhdxに対して`compact vdisk`を実行する（pruneだけでは縮まない）。
4. 開発中は使わないコンテナを`docker compose stop`で止め、常時全部立てっぱなしにしない。
